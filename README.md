...
That means the add-in likely failed during startup before it finished creating the toolbar button. This is different from the earlier case where the add-in loaded successfully but you had not clicked the toolbar button yet.

Use this checklist:

1. Make sure you replaced the entire old add-in folder, not just one file.
2. Make sure the folder contains both `LatticeBedsAddin.py` and `LatticeBedsAddin.manifest`.
3. Start the add-in again and watch for a startup message box.
4. Open **View > Text Commands** and look for lines beginning with `[LatticeBedsAddin]`.
5. If the button still does not appear, stop the add-in, remove the linked folder from Fusion, then add the new folder again as a fresh link.

The startup log should show one of these states:

- `Starting add-in...`
- `Created command definition.` or `Reusing existing command definition.`
- `Added toolbar button to Solid > Scripts and Add-Ins.`

If you do not see those lines, Fusion is not loading the folder you think it is, or the add-in is failing before UI registration completes.
